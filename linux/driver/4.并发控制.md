...
- 异常上下文(不可阻塞:sleep)
```c
in_interruprt()//是否在 软/硬 中断上下文
in_atomic()//是否在 软/硬 中断上下文+持有 spinlock，持有spinlock 是可以在 进程上下文发生的
...
```
- 根据 使用上下文（进程上下文或者中断上下文），选取合适的版本使用
...
## 信号量
- P&V, sem=0时 sleep(block),否则P 减1，, V+1,然后唤醒睡眠
...
```
     禁止中断		              使能中断
local_irq_disable();	    local_irq_enable();			禁用，启用 本cpu 上的中断

local_irq_save(flags);		local_irq_restore(flags);	flage与cpu的中断位相关, flags 存储中断信息
