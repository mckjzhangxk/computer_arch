...
//    local_addr.sin_addr.s_addr = INADDR_ANY;//0.0.0.0
    ...
    //////////////////////////////监听套接字///////////////////////////
    int backlog = 2; //并发缓冲区大小，可以理解为并发量
    ...
    getsockname(socket_fd, reinterpret_cast<sockaddr *>(&local_addr), &len);
    int rport=ntohs(local_addr.sin_port);
    printf("port %d\n",rport);
...
int main(int argc,char* argv[]){
    ...
    int socket_fd=open_listen_fd("0.0.0.0",argv[1]);
